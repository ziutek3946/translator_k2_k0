...
//wyslanie linii z konsoli
void on_line(elcc::editor & el, std::string const& line)
{
    //zapisanie wprowadzonej linii do historii
	el.history().enter(line);
	//wprowadzenie linii do parsera ktory przetlumaczy komunikat z k2 na k0 i opublikuje go w mqtt
	Parser p(line);
}
...
//autouzupelnianie
elcc::word_list completion_handler(const elcc::word_list& lista, size_t idx)
{
    Cfg_reader cfg;
    std::vector<std::string> cmds;

    //kolejne poziomy podpowiedzi
	switch (idx) {
	case 0:
		return cfg.GetCommand();
	case 1:
		return cfg.GetDevice(lista[idx - 1]);
    default:
        break;
	}
